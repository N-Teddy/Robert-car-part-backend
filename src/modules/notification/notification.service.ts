...
export class NotificationService {
  ...
  async sendNotification(dto: SendNotificationDto): Promise<SendNotificationResultDto> {
    ...
    if (dto.channel === NotificationChannelEnum.WEBSOCKET || dto.channel === NotificationChannelEnum.BOTH) {
    ...
    if (dto.channel === NotificationChannelEnum.EMAIL || dto.channel === NotificationChannelEnum.BOTH) {
  ...
  async batchSendNotifications(dto: BatchSendNotificationDto): Promise<BatchSendResultDto> {
  ...
  async markAsRead(dto: MarkAsReadDto, userId: string): Promise<void> {
    ...
    if (notifications.length === 0) {
      throw new BadRequestException('No notifications found to mark as read');
    }
  ...
  async getNotifications(filter: NotificationFilterDto): Promise<NotificationResponseDto[]> {
  ...
  async getNotificationById(id: string, userId: string): Promise<NotificationResponseDto> {
    ...
    if (notification.user && notification.user.id !== userId) {
      ...
      if (!user || (user.role !== UserRoleEnum.ADMIN && user.role !== UserRoleEnum.MANAGER)) {
    ...
    switch (audience) {
      ...
      case NotificationAudienceEnum.ADMIN:
        ...
        query = query.where('user.role IN (:...roles)', {
          roles: [UserRoleEnum.DEV, UserRoleEnum.MANAGER, UserRoleEnum.ADMIN],
        });
        break;
      ...
      case NotificationAudienceEnum.STAFF:
        // STAFF and above
        query = query.where('user.role IN (:...roles)', {
          roles: [UserRoleEnum.STAFF, UserRoleEnum.MANAGER, UserRoleEnum.ADMIN],
        });
        break;

      case NotificationAudienceEnum.SPECIFIC_USER:
        if (!userIds || userIds.length === 0) {
          throw new BadRequestException('User IDs required for SPECIFIC_USER audience');
        }
  ...
  private getActionUrl(type: NotificationEnum, metadata?: Record<string, any>): string {
