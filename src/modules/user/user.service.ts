...
export class UserService {
    ...
    async findAllStaff(filters: StaffFilterDto): Promise<{ users: User[]; total: number }> {
        try {
    ...
    async updateStaff(userId: string, updateStaffDto: UpdateStaffDto): Promise<User> {
        try {
            ...
            if (updateStaffDto.role && updateStaffDto.role !== UserRoleEnum.ADMIN && user.role === UserRoleEnum.ADMIN) {
    ...
    async deactivateStaff(userId: string): Promise<User> {
        try {
            ...
            throw new InternalServerErrorException('Failed to deactivate staff member');
        }
    }
    ...
    async activateStaff(userId: string): Promise<User> {
        try {
            ...
            throw new InternalServerErrorException('Failed to activate staff member');
        }
    }
    ...
    async getActiveStaff(): Promise<User[]> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch active staff members');
        }
    }
}