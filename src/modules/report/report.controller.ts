...
export class ReportController {
    ...
    async create(@Body() generateReportDto: GenerateReportDto): Promise<ReportResponseDto> {
    ...
    async findAll(@Query() query: ReportQueryDto): Promise<{ data: ReportResponseDto[]; total: number }> {
        return this.reportService.findAll(query);
    }

    @Get('stats')
    @ApiOperation({ summary: 'Get report statistics' })
    @ApiResponse({ status: 200, description: 'Report statistics', type: ReportStatsResponseDto })
