...
export class UploadService {
	...
	private async validateEntity(
		...
		try {
				...
				case ImageEnum.CATEGORY:
					...
					if (!category) {
						throw new NotFoundException(
							`Category with ID ${entityId} not found`
						);
					}
					if (category.image) {
			...
			this.logger.error(
				`Failed to validate entity: ${entityType} - ${entityId}`,
				error
			);
	...
	async uploadSingleImage(
		...
		try {
			...
			// Validate file size
			const maxSize =
				this.configService.get<number>('MAX_FILE_SIZE') || 10485760; // 10MB default
			...
			switch (entityType) {
				case ImageEnum.USER_PROFILE:
					imageData.user = { id: entityId } as any;
					break;
				case ImageEnum.VEHICLE:
					imageData.vehicle = { id: entityId } as any;
					break;
				case ImageEnum.PART:
					imageData.part = { id: entityId } as any;
					break;
				case ImageEnum.CATEGORY:
					imageData.category = { id: entityId } as any;
					break;
				case ImageEnum.QR_CODE:
					imageData.qrCode = { id: entityId } as any;
					break;
			}

			// Create and save the image
			const image: Image = this.imageRepository.create(imageData);
	...
	async getImagesByEntity(
		...
		try {
			...
			this.logger.error(
				`Failed to get images for entity: ${entityType} - ${entityId}`,
				error
			);
