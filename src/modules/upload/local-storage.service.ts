...
export class LocalStorageService {
    ...
    async uploadImage(
        ...
        try {
            ...
            const typeDir = join(this.uploadsDir, imageType.toLowerCase().replace(' ', '-'));
    ...
    async getImageStats(): Promise<{ totalFiles: number; totalSize: number; typeBreakdown: Record<string, number> }> {
        try {
    ...
    private async getDirectoryStats(dirPath: string): Promise<{ totalFiles: number; totalSize: number; typeBreakdown: Record<string, number> }> {
        let totalFiles = 0;
        let totalSize = 0;
