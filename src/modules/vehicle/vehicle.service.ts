...
import { NotificationAudienceEnum, NotificationChannelEnum, NotificationEnum } from '../../common/enum/notification.enum';
...
export class VehicleService {
    ...
    async create(
        ...
        try {
            ...
            const savedVehicle: Vehicle = await this.vehicleRepository.save(vehicle);
    ...
    async findAll(
        ...
        maxYear?: number,
        isPartedOut?: boolean,
    ): Promise<{ data: VehicleResponseDto[]; total: number; page: number; limit: number; totalPages: number; hasNext: boolean; hasPrev: boolean }> {
        try {
            ...
            if (model) {
                options.where = { ...options.where, model: ILike(`%${model}%`) };
            }
            ...
            if (minYear !== undefined && maxYear !== undefined) {
                options.where = { ...options.where, year: Between(minYear, maxYear) };
            } else if (minYear !== undefined) {
                options.where = { ...options.where, year: MoreThanOrEqual(minYear) };
            } else if (maxYear !== undefined) {
                options.where = { ...options.where, year: LessThanOrEqual(maxYear) };
            }
    ...
    async remove(id: string, userId: string): Promise<{ success: true }> {
        try {
            ...
            if (vehicle.parts && vehicle.parts.length > 0) {
                throw new BadRequestException('Cannot delete vehicle with associated parts');
            }

            // Delete associated images
            if (vehicle.images && vehicle.images.length > 0) {
                await Promise.all(
                    vehicle.images.map(image => this.uploadService.deleteImage(image.id))
                );
            }
    ...
    async markAsPartedOut(id: string, userId: string): Promise<VehicleResponseDto> {
        try {
            const vehicle = await this.vehicleRepository.findOne({
                where: { id },
                relations: ['images'],
            ...
            throw new InternalServerErrorException('Failed to mark vehicle as parted out');
        }
    }
    ...
    async getStatistics(): Promise<any> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch vehicle statistics');
        }
    }
    ...
    async getMakeModelStatistics(): Promise<any> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch make/model statistics');
        }
    }
}