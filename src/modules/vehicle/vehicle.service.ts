...
export class VehicleService {
    ...
    async createVehiclesBulk(bulkCreateDto: BulkCreateVehicleDto, userId: string): Promise<any[]> {
        const results = [];
    ...
    async updateVehicle(id: string, updateVehicleDto: UpdateVehicleDto, userId: string): Promise<Vehicle> {
        try {
    ...
    async updateVehiclesBulk(bulkUpdateDto: BulkUpdateVehicleDto, userId: string): Promise<any[]> {
        const results = [];
    ...
    async deleteVehicle(id: string, userId: string): Promise<void> {
        try {
            ...
            if (vehicle.parts && vehicle.parts.length > 0) {
                throw new BadRequestException('Cannot delete vehicle with existing parts');
            }
    ...
    async markAsPartedOut(id: string, userId: string): Promise<Vehicle> {
        try {
            ...
            if (vehicle.isPartedOut) {
                throw new BadRequestException('Vehicle is already marked as parted out');
            }
    ...
    private async calculateVehicleStats(vehicle: Vehicle): Promise<VehicleWithStats> {
        try {
            // Get parts stats
            const partsStats = await this.partRepository
                .createQueryBuilder('part')
    ...
    private buildSearchQuery(searchDto: VehicleSearchDto): SelectQueryBuilder<Vehicle> {
