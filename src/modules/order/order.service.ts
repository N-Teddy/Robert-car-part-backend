...
import { NotificationAudienceEnum, NotificationChannelEnum, NotificationEnum } from 'src/common/enum/notification.enum';
...
export class OrdersService {
    ...
    async findAll(query: OrderQueryDto): Promise<{ data: OrderResponseDto[]; total: number }> {
        const {
            page,
            limit,
            status,
    ...
    async getStats(): Promise<OrderStatsResponseDto> {
        ...
        const totalRevenueResult = await this.orderRepository
            ...
            .where('order.status = :status', { status: OrderStatusEnum.COMPLETED })
            .getRawOne();
    ...
    async generateReceipt(orderId: string): Promise<Buffer> {
        ...
        if (order.status !== OrderStatusEnum.COMPLETED) {
            throw new BadRequestException('Receipt can only be generated for completed orders');
        }
