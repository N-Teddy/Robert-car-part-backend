...
import { Repository, FindManyOptions, ILike, Between, MoreThanOrEqual, LessThanOrEqual, In } from 'typeorm';
...
export class PartService {
    ...
    private async generateQrCodeForPart(part: Part, userId: string): Promise<void> {
        try {
    ...
    async findAll(
        ...
        maxQuantity?: number,
        condition?: string,
    ): Promise<{ data: PartResponseDto[]; total: number; page: number; limit: number; totalPages: number; hasNext: boolean; hasPrev: boolean }> {
        try {
    ...
    async remove(id: string, userId: string): Promise<{ success: true }> {
        ...
        try {
            ...
            if (part.orderItems && part.orderItems.length > 0) {
                throw new BadRequestException('Cannot delete part with associated orders');
            }
    ...
    async getStatistics(): Promise<any> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch part statistics');
        }
    }
    ...
    async getCategoryStatistics(): Promise<any> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch category statistics');
        }
    }
    ...
    async getLowStockParts(): Promise<PartResponseDto[]> {
        try {
            ...
            throw new InternalServerErrorException('Failed to fetch low stock parts');
        }
    }
}